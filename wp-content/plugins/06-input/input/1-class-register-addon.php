...
class register_addon extends database {
      ...
      if ( $addon_files ) {
        foreach ( $addon_files as $addon_file ) {
          ...
          if ( isset( $sst ) ) {
            ...
            if ( $this->check_is_in_table( array( 'func' => $sst[ 'function' ] ), $GLOBALS[ 'sst_tables' ][ 'input_type' ] ) === false ) {
  ...
  function register_field_type( string $html_type, string $type, $function, $epithet = NULL, $description = NULL, $slug = NULL, $owner = NULL, $id = NULL ) {
	...
	$reg = $this->add_to_table( $GLOBALS[ 'sst_tables' ][ 'input_type' ], array( 'id' => $id, 'epithet' => $epithet, 'slug' => $slug, 'html_type' => $html_type, 'type' => $type, 'func' => $function, 'description' => $description, 'owner' => $owner, 'created' => date( 'Y-m-d H:i:s' ), 'modified' => date( 'Y-m-d H:i:s' ) ) );
	  if($reg['result']==true){
