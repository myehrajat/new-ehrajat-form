...
jQuery(document).ready(function () {
	...
	function change_input_names(sst){
		...
		while(sst.form_inputs.length>sst.i){
			...
			//this change id of input for new created element
			jQuery(sst.form_inputs[sst.i]).attr('id',jQuery(sst.form_inputs[sst.i]).attr('id').split('≪')[0].concat(sst.target_identifier));
			sst.i++;
		}
		
	}
	function change_input_labels_for_attr(sst){
		sst.form_input_labels = jQuery('#'+sst.new_clone_target_id).find("label[for]");
		console.log(sst.form_input_labels);
		sst.source_identifier_length = sst.source_identifier.length;
		sst.i=0;
		while(sst.form_input_labels.length>sst.i){
			jQuery(sst.form_input_labels[sst.i]).attr('for',jQuery(sst.form_input_labels[sst.i]).attr('for').split('≪')[0].concat(sst.target_identifier));
		}
	}
	var sst = {};
	 ...
	 jQuery('body').on('click',"[id$='"+sst.add_contoller_prefix+"']",function(event){
		 ...
		 if(sst.max_extra> sst.clone_source_last_number ){
			...
			// }
			 ...
			 change_input_labels_for_attr(sst);
			 	//change_input_ids(sst);
				 jQuery(this).hide();
