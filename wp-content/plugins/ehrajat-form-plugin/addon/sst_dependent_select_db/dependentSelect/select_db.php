...
function sst_create_options_db($standard_db_id,$return_data_only=false,$where_replace=false,$value_to_replace = NULL){
	...
	if($obj_options){
		...
		foreach($obj_options as $obj_option){
			...
			$label_value = sst_access_cols($obj_option,$label,$label_function);
			$value_value = sst_access_cols($obj_option,$value,$value_function);
			$id_value = sst_access_cols($obj_option,$id,$id_function);
...
*/
function sst_access_cols($obj_option,$col_names,$function){
