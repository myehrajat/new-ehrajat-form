...
class field {
	...
	function get_field_object($field_id){
		...
		if ( !empty( $field_id ) ) {
            ...
            sst_error_log( 'field id is empty or is now positive int.' );
            return NULL;
        }
	}
    function create_inputs() {
            if ( !empty( $this->field_obj ) ) {
                ...
                if ( !empty( $input_ids ) ) {
                    ...
                    sst_error_log( 'no input ids after processing input ids of your field.' );
                    return NULL;
                }
            } else {
                sst_error_log( 'field id is empty or after processing is empty.' );
                return NULL;
            }
    ...
    function render() {
       $tag_id =  $this->common->get_ids( $this->field_obj->tag_id,true );
		if(!empty($tag_id)){
            ...
            sst_error_log( 'tag id provided for field can retireve object.' );
            return NULL;
			}
		}else{
            return NULL;
			...
			if(!empty($this->user_access_obj)){
				...
				switch($default_show_or_hide){
					...
					case 'hide':
						...
						break;
					default:
						sst_error_log( 'default_show_or_hide is incorect even DEFAULT_SHOW_OR_HIDE in config is incorect.' );
						$this->show_or_hide = 'hide';
						break;
				}
				...
				switch($default_enable_or_disable){
					...
					case 'disable':
						...
						break;
					default:
						sst_error_log( 'default_enable_or_disable is incorect even DEFAULT_ENABLE_OR_DISABLE in config is incorect.' );
						$this->enable_or_disable = 'hide';
						break;
				}
			}else{
				sst_error_log( 'user_access_id cant fitnd its obj.' );
				return NULL;
			}
		}else{
            sst_error_log( 'user_access_id after proceesing return enpty id.' );
            return NULL;
		}
